...
def get_delim_csv(file_path):
    """
    Detect the delimiter of a CSV file by analyzing the first line.
    
    Args:
        file_path (str): Path to the CSV file
        
    Returns:
        str: Detected delimiter (e.g., ',', ';', '\t')
    """
...
def extract_columns_from_excel(file_path, columns):
    ...
    try:
        ...
        if file_path.endswith(".xlsx"):
            ...
            df = pd.read_csv(file_path, header=0, dtype=str, encoding='utf-8-sig', delimiter=get_delim_csv(file_path))
            df.columns = [col.strip() for col in df.columns]
        # Check if all specified columns exist
        for col in columns:
            if col not in df.columns:
                raise Exception(f"Error: Column '{col}' not found in the file.\ncolumns found in the file: {df.columns.tolist()}")
        
        # Return only columns that exist in the DataFrame
        existing_columns = [col for col in columns if col in df.columns]
        return df[existing_columns]
    except Exception as e:
        raise Exception(f"Error reading file: {e}") from None
...
def field_length_check(row, columns, min_lengths,max_lengths,weights):
    ...
    for col,weight in zip(columns,weights):
            if (isinstance(row[col], str) and min_lengths <= len(row[col]) <= max_lengths) or (pd.isna(row[col]) or len(row[col])==0):
                total_weight += weight
...
def grade_calculation(df, columns, common_words, weights,min_lengths=3,max_lengths=50):
    ...
    inversed_common_words={col:[] for col in common_words.keys()}
    for col in common_words.keys():
        for _, words in common_words.items():
            if _!=col:
                inversed_common_words[col].extend(words)
    for i in range(len(df)):
        # Check for keywords in the specified columns
        grades[i] += row_keyword_check(df.iloc[i], weights['keywords'], columns[1:], common_words,True)
        # Check for keywords not in the specified columns
        grades[i] += row_keyword_check(df.iloc[i], weights['wrong_keywords'], columns[1:], inversed_common_words,False)
        # Field length check
...
def row_keyword_check(row, weight, list_of_columns, keywords_dict,add_if_find=True):
    """
...
def grading_main(verification_parameters):
    ...
    columns=[col.strip() for col in verification_parameters["input_columns"]]
